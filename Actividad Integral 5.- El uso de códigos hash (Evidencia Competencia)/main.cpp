...
HashTable<string, Pokemon> hash1(151, string (""), myHash);

List<Pokemon> lista;

Heap<string> numerosHeap(151);
Heap<string> nombresPoke(151);

int numeroPokemon;
string numPoke, nombrePokemon, tipoPokemon, nombrePoke;
string opcioncase5;
...
int main() 
{
  ...
  while (myfileout>>numeroPokemon>>nombrePokemon>>tipoPokemon>>nombrePokemon>>numeroPokemon){
   ...
   cout<<"5. Buscar datos de pokemon por medio de tabla hash"<<endl;
   cout<<"6. Salir"<<endl;
   cout<<"\n";
   cin>>opcion;
   cout<<"\n";
   ...
   switch(opcion)
   {
     ...
     case 5:
      ...
      cout << "Cargando nombres de los Pokemones de Kanto . . ." << endl;
      cout<<""<<endl;
      myfileout.open("nombresPoke.txt");
	    if (myfileout.is_open()) {
        while(getline(myfileout,line)) {
          stringstream stream(line);
          cout << "Nombres cargados satisfactoriamente\n";
          while(getline(stream,nombrePoke,' ')) {
            for(int i = 0; i < vector_pokemon.size(); i++){
              if(vector_pokemon[i].get_nombre() == nombrePoke){
              hash1.put(nombrePoke, vector_pokemon[i]);
              }
            }
          }
        }
      ...
      myfileout.close();
      cout<<""<<endl;
      cout<<"¿Que pokemon de la region de Kanto quieres buscar? (Recuerda escribir la primera letra en mayuscula ej. Pikachu)"<<endl;
      cout<<""<<endl;
      ...
      if(hash1.contains(opcioncase5))
      { 
        Pokemon pokeEncontrado;
        cout<< "Pokemon encontrado!"<<endl;
        pokeEncontrado = hash1.get(opcioncase5);

          if (myfilein.is_open())
          {
          myfilein<<"---------------------------"<<endl<<"No. "<<pokeEncontrado.get_numero()<<" "<<pokeEncontrado.get_nombre()<<endl<<"Type"<<endl<<"\t\t"<<pokeEncontrado.get_tipo()<<endl<< "Weight"<<" .......... "<<pokeEncontrado.get_peso()<<" Kg"<<endl<<"Height"<<" .......... "<<pokeEncontrado.get_altura()<<" Mts"<<endl<<"---------------------------"<<endl;
          }
        cout<<"\n";
        cout<<"Imprimiendo datos"<<endl;
